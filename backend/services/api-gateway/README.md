...
- OpenTelemetry → Jaeger (`OTEL_ENABLED=true`, см. `backend/docs/observability.md`);
...
1. Поднять `backend/infra/compose` и запустить `auth-service`.
...
1. Redis из `backend/infra/compose` (для `THROTTLE_STORAGE=redis`).
