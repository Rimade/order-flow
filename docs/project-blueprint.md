...
Следующим практическим шагом считаем создание skeleton-структуры репозитория:

- `infra/compose` для локального окружения;
- `services/` для сервисов на `NestJS` и `Go`;
- базовый `docker-compose`;
- стартовые README по каждому сервису.
